...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start time.Time, end time.Time) ([]models.Room, error) {
	...
	query := `
		SELECT r.id, r.room_name FROM rooms r WHERE r.id NOT IN (SELECT room_id from room_restrictions rr WHERE $1 < rr.end_date AND $2 > rr.start_date)
	`
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start time.Time, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3 * time.Second)
	defer cancel()

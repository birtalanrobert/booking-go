...
var theTests = []struct {
	...
	{"non-existent", "/non/existing/url/path", "GET", http.StatusNotFound},
...
func TestReservation(t *testing.T) {
	for _, e := range reservationTests {
		...
		if rr.Code != e.expectedStatusCode {
			t.Errorf("%s returned wrong response code: got %d, expected %d", e.name, rr.Code, e.expectedStatusCode)
		}
...
func TestPostReservation(t *testing.T) {
	for _, e := range postReservationTests {
		...
		if e.postData != nil {
			req, _ = http.NewRequest("POST", "/make-reservation", strings.NewReader(e.postData.Encode()))
		} else {
...
func TestAvailabilityJSON(t *testing.T) {
	for _, e := range testAvailabilityJSONData {
		...
		if e.postData != nil {
			req, _ = http.NewRequest("POST", "/search-availability-json", strings.NewReader(e.postData.Encode()))
		} else {
...
func TestPostAvailability(t *testing.T) {
	for _, e := range testPostAvailabilityData {
		req, _ := http.NewRequest("POST", "/search-availability", strings.NewReader(e.postData.Encode()))
...
// TestPostReservationCalendar tests for PostReservationCalendar handler
